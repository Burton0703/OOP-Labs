// handles previews and calls shape constructors
...
void RectangleEditor::OnMouseMove(HWND hWnd)
{
	...
	hPen = CreatePen(PS_DOT, 1, black);
	hPenOld = (HPEN)SelectObject(hdc, hPen);
	Rectangle(hdc, x1, y1, x2, y2);//2 * x1 - x2, 2 * y1 - y2, x2, y2
	GetCursorPos(&pt);
	ScreenToClient(hWnd, &pt);
	x2 = pt.x;
	y2 = pt.y;
...
void EllipseEditor::OnLMBup(HWND hWnd)
{
	...
	Ellipse->Set(2 * x1 - x2, 2 * y1 - y2, x2, y2);//x1, y1, x2, y2
